    ...
    def get_market_data(self, instrument_type, symbol, data_type, timeframe=None, duration=None, expiry=None, strike=None, right=None):
        ...
        try:
            ...
            if data_type == "OHLC":
                ...
                logger.info(f"Starting historical data collection for {symbol} from {end} back to {start_time}")
                ...
                while current_end > start_time:
                    try:
                        ...
                        logger.info(f"Requesting data for period ending at {current_end}")
                        ...
                        ticks = self.ib_connection.ib.reqHistoricalTicks(
                            ...
                            '',  # Start time empty
                            current_end,
                            1000,  # Maximum ticks per request
                            'TRADES',
                            useRth=True  # Only regular trading hours
                        )
                        
                        if not ticks:
                            logger.warning(f"No ticks returned for period ending at {current_end}")
                            if total_ticks == 0:  # If this is our first attempt and we got no data
                                break
                            # Move back by 1 hour and try again
                            current_end = current_end - pd.Timedelta(hours=1)
                            continue
                        
                        # Update the end time for next request
                        # Make sure the time from ticks is timezone-aware
                        current_end = ticks[0].time.replace(tzinfo=timezone.utc) if ticks[0].time.tzinfo is None else ticks[0].time
                        total_ticks += len(ticks)
                        
                        # Prepend ticks to our list
                        all_ticks = ticks + all_ticks
                        
                        logger.info(f"Retrieved {len(ticks)} ticks. Total ticks so far: {total_ticks}")
                        
                    except Exception as e:
                        logger.error(f"Error retrieving historical ticks: {str(e)}")
                        self.ib_connection.ib.sleep(5)  # Longer wait if we hit an error
                        continue
                    
                    # Break if we've reached our target start time
                ...
                # Filter data to match requested duration
                ...
                logger.info(f"Retrieved total of {len(df)} ticks between {start_time} and {end}")
